...
const templateWithJQuery = () => {
 ...
 starWarsPeople.forEach(element => {
   let getArr = $('#template').html();
   let $newArr = $(`<section>${getArr}</section>`);
   $newArr.find('h2').text(element.name);
   $newArr.find('h3').text(element.height);
   $newArr.find('p').text(element.eye_color);
   $('main').append($newArr);
 });
}
