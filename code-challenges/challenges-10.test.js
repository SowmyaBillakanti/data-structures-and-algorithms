...
const createServer = () => {
  ...
  app.get('/hello', (request, response) => response.send('Greetings, Hello!'));
  app.get('/aboutme', (request, response) => response.send('I\'m Sowmya, I am a student at Codefellows'));
  app.get('/favoritefoods', (req, res) => res.send(['Ice-creams, Chocolates, Indian food']));
  app.get('*', (req, res) => res.status(404).send('Could not find route.'));
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map((element) => {
    return element.filter((div) => {
      if (typeof div === 'number' && div % 5 === 0) {
        return true;
      }
    }).map((ele) => {
